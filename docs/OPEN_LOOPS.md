...
| L11 | ถอด Guestbook ออกจาก nav ใน `BaseLayout.astro` (D5) | Claude | P2 | Lab 04 redo (#8) | หน้า guestbook ใช้งานได้แล้วแต่อยู่นอก v1 |
| L12 | Rate limit `POST /api/guestbook` แบบเดียวกับ contact | OpenCode | P3 | ก่อนเปิด guestbook | จาก `docs/fe-be-contract-check.md` |
| L13 | Contact 429: ตัด suffix "ลองใหม่" ซ้ำ · (optional) แสดงเวลาจาก `retry-after` | Claude | P3 | Lab 04 redo / 06 | จาก `docs/fe-be-contract-check.md` |
...
| ID | Task | Closed |
|---|---|---|
| L9 | honeypot `website` + ถอด API path ออกจากฟอร์ม · maxlength ตรง `LIMITS` | 2026-09-25 (Lab 05b swarm) |
