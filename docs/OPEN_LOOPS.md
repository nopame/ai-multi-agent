...
| ID | Task | Closed |
|---|---|---|
| L10 | Proxy-aware client IP (ใช้ entry ขวาสุดของ `x-forwarded-for` + fallback `x-real-ip`) + bucket pruning ใน `checkRateLimit` | 2026-09-25 (branch `lab-05-backend` · test ครบ) |
| L12 | Rate limit `POST /api/guestbook` แบบเดียวกับ contact (429 + `retry-after`) | 2026-09-25 (branch `lab-05-backend` · `tests/guestbook-api.test.ts`) |
